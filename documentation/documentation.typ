...
==== Visual Studio Code
#acr("VsCode") is a code editor developed by Microsoft with a #acr("FOSS") core, that also contains proprietary components such as telemetry. Although it is not a full-fledged #acr("IDE"), as its core does not contain tools typically found in an #acr("IDE"), such as debuggers and code completion, it is still a very powerful tool thanks to its expansive and well-supported extension eco-system. It is thanks to those extensions, that #acr("VsCode") has become very popular in Web Development, as many developers are dependent on #acr("VsCode") extensions for working with popular frameworks and libraries. When working with very popular technologies, no extensions are needed, since Microsoft has included tooling for web technologies out of the box such as a basic debugger and code highlighting for the popular programming languages JavaScript and Python @ms-vscode as well as EMMET, an extension providing productivity-enhancing abbreviations and snippets for #acr("HTML") and #acr("CSS"). @ms-emmet

In this project, Visual Studio was primarily used for the Frontend of the application.
==== Docker Desktop/Docker
Docker Desktop is a #acr("GUI") application for Windows and macOs for managing Docker Containers. It is available in both a #acr("FOSS") community edition and a paid commercial version. It also allows the developer to create and maintain Docker Images as well as Kubernetes Clusters.

Docker itself is a system that allows developers to quickly deploy software. It might be easier to imagine it as a way to package an application, including all dependencies and configuration files into a self-contained and reusable package. Said package is referred to as a Container. It is often used in combination with a #acr("CI/CD") pipeline, where code is automatically tested, built, and in the case of Continuous Deployment published. Images are the core of Docker. Images contain instructions on how to build Containers, so it is possible to think of them as "Blueprints" for creating Containers. The instructions are defined in a Dockerfile containing information about the Container #acr("OS"), startup commands, and software dependencies. Containers are instances of Docker images. As Containers are isolated and self-contained, it is possible to run multiple instances at the same time. This also helps improve the stability and security of the software.

Even if the developer does not containerize their software, as is the case with this project, Docker can still be a useful tool, because many common dependencies such as Database are available as docker images, which removes the need to install them locally. @misc-dockerdesktop
=== Other Software
This section outlines software that was used for various miscellaneous tasks other than than software development.
==== Mozilla Firefox
Mozilla Firefox is a #acr("FOSS") web browser developed by the Mozilla Corporation. It began development in 1998 as a fork of the then open-source Netscape Navigator Browser, which had at one point been the most popular web browser in the early days of the internet. It had, however, lost a great bit of popularity at the time it was made open-source. Firefox in turn became a very popular browser in the 2000s, but it has also seen declining usage for the last couple of years as the Chrome Browser, developed by Google, has taken the lead. Firefox now hovers around 3 to 10% market share depending on the region.

Firefox was chosen for this project to test and interact with the Frontend interface. It was preferred over Chrome or Browser based on it, since it not only offers a greater amount of privacy but also better developer tools, especially for viewing #acr("CSS") properties of #acr("HTML") elements. @eb-firefox
==== Swagger #acr("UI")
Swagger #acr("UI") is an interface built upon the OpenAPI Specification for documenting an #acr("API") by using either a YAML or #acr("JSON") document to describe the available endpoints and the models used for data transfer. It was formally known as the Swagger Specification. Swagger itself includes a set of tools, which aid the developer in the design, development, and documentation of #acr("API")s.

Swagger #acr("UI") is one of these tools. It helps the developer keep an overview of the #acr("API") by generating a #acr("GUI") out of the YAML/#acr("JSON") document displaying all endpoints. It also allows for the testing of said endpoints by providing fields for each parameter of an endpoint, where test data can be entered and buttons to complete the #acr("API") calls. @misc-swagger
==== Typst
Typst is a newly developed #acr("FOSS") markup system primarily targeted toward academic use. It is intended to replace LaTeX. Typst is inspired by Markdown as it includes built-in syntax for commonly used text elements like bold/italic text, unordered/ordered lists, and headings. Typst is entirely built with Rust, a modern type-safe high-performance programming language resulting in very fast compile times compared to LaTeX.

A major difference between Typst and older solutions, however, is not separated into a distinct markup and scripting language but is instead one language containing both capabilities. Its scripting side includes implementations of conditionals, loops, and data types including Strings, Integers, Floats, Arrays, and Dictionaries, all of which are commonly found in other programming languages making Typst very intuitive for programmers, while not being too complicated for regular users. Additionally, it supports many formats for storing bibliographies, including Hayagriva YAML and BibLaTeX files. Typst documents can be exported as #acr("PDF") and #acr("HTML") files.

The markup and scripting components of Typst are very closely linked and they can both be embedded in each other. Declared variables are also shared between the two, making it very easy to for example create a list of names, programmatically modify that list, and output the result in the space of a couple of lines in one file. The same procedure would require the use of multiple separate files in comparable systems. Typst also allows the user to programmatically change the properties of text elements anywhere in the document by using so-called Set Rules. The following two examples show how to add numberings to headings and how to justify the text of paragraphs:
```typ
...
Typst also allows for the creation of document-independent templates and packages. Even though Typst is still very new, there is already a thriving community of developers and general users creating said components and sharing them for free with the rest of the user base.
...
- *Repository*: Repositories represent the fundamental component of Git. They are used to store the code of a project or part of a project. Changes to files are also tracked on the repository level. On the technical level, it is a special database storing project revisions and history. Unlike other similar solutions, a Git repos not only contains file data but also data about itself such as metadata and user configuration. That information is located in a ".git" directory, which is configured to be hidden. Git uses two different key structures to store data: the object store (efficiently copied during cloning) and the index (temporary and modifiable).
...
==== SignalR
In a traditional Web #acr("API"), clients can only send requests to the server, to which said server may then respond. This system works well for certain use cases, but for others, for example, chats or games, it is too limited. Instead, these applications may benefit from the use of another technology, namely WebSockets. They differ from traditional #acr("HTTP") endpoints by establishing a persistent connection between clients and servers with two-way communication channels. The technical term for communication of that sort would be "Full-Duplex Communication". Messages transmitted over WebSockets are also delivered or received in real time. Although their usefulness is without doubt, WebSockets also possess certain limitations, which a developer has to keep in mind when using them:
...
As a solution to said problems, Microsoft released the SignalR library for .NET and .NET Core. It enables developers to create easily. At the core of SignalR are so-called Hubs. There, functions can be created, which correspond to a certain task performed by the application. Within them, business logic is defined and Model data is interacted with. Should the changes to the application state performed in a function require updates in the clients, messages can be sent to either all clients, groups of clients, or specific clients. SignalR is also very easily combined with Microsoft Identity to associate specific clients with logged-in users.
...
=== PostgreSQL
The database used for this project is PostgreSQL, a #acr("FOSS") relational database. Similar to other relational databases like MySQL and SQLServer, it stores data in tables divided into rows and columns. Another shared characteristic is that most relational databases use #acr("SQL") to allow a developer to write queries performing #acr("CRUD") operations on stored data. What differentiates PostgreSQL from other solutions is that it is a so-called Object Relational Database, meaning that it supports concepts like custom data types and inheritance typically found in #acr("OOP"). It also allows multiple transactions to run at the same time, by assigning a snapshot of the database to each one and for reusable queries. @misc-postgres
...
==== DTOs
In software development, a #acr("DTO") is like a specialized messenger carrying only the data needed between different parts of an application. In their implementation, #acr("DTO")s look very similar to regular models. The major difference is that they are not meant to represent data stored on the database, but instead data sent and received. A #acr("DTO") is usually associated with a model and includes all fields from said model needed for a certain data transfer. They are also a good way to obfuscate data and are categorized as a Design Pattern. @misc-dto
